...
- Zakładka **Wartość aktywów**: RAP 1 | RAP 2 (`st.dataframe` + Pandas Styler: kolory portfeli / EUR / PLN / `Z RAZEM`; kwoty do prawej; etykiety indeksu blankowane jak w CLI — portfel tylko przy pierwszym wystąpieniu; CLI `format_rap_table`), potem Cash pool (jedna tabela), potem **Inwestycje** jako trzy tabele — `0 OGÓLNY`, `1 REVOLUT-ROBO`, `2 G-MOMENTUM`.
