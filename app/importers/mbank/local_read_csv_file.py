...
def read_mbank_csv_file(input_file: Path, **arg) -> Tuple[pd.DataFrame, str]:
    options = (
        {},
    ...
    for arg in options:
        try:
            result, ref_date = _read_mbank_csv_file(input_file, **arg)
            return result, ref_date
        except UnicodeDecodeError:
            continue
    raise ValueError


def _read_mbank_csv_file(input_file: Path, **arg) ->  Tuple[pd.DataFrame, str]:
    with open(input_file, 'r', **arg) as f:   # , encoding='Windows-1250'
        ...
        base_account, ref_date, result = extract_csv_table(str_io)

    result = pd.DataFrame(result)
    result = result.drop(columns=[''], errors='ignore')
