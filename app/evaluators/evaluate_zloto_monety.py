...
def evaluate_zloto_monety(
    ...
    outcomes = _match_all_purchase_rules(data_root, assets_catalog, purchase_rules, warnings)
...
def _match_all_purchase_rules(
    ...
    for source_account, rules in purchase_rules.groupby(GoldCoinPurchaseRules.SOURCE_ACCOUNT):
        ...
        transactions = _load_source_transactions(data_root, source_row)
...
def _load_source_transactions(data_root: Path, source_row: pd.Series) -> pd.DataFrame:
