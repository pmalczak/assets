...
from global_momentum_u8_ranking import RANKING_TICKERS, compute_current_universe8_ranking
...
class GlobalMomentumRankingTests(unittest.TestCase):
    def test_ready_ranking_has_top3_allocation(self):
        result = compute_current_universe8_ranking(
            _monthly_prices(),
            list(RANKING_TICKERS.keys()),
        )
        self.assertTrue(result["ready"])
        self.assertEqual(len(result["ranking"]), len(RANKING_TICKERS))
        self.assertEqual(set(result["ranking"]["Rank"]), set(range(1, 9)))
    ...
    def test_unavailable_signal_returns_availability(self):
        ...
        result = compute_current_universe8_ranking(short, list(RANKING_TICKERS.keys()))
